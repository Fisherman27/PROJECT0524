import { PreReplayReport, PostReplayReport, ReportBullet, RiskItem, RiskRadarItem, EvidenceCard, ReplayCard, AgentTraceItem } from "@/types/replay";
...
function mdCards(items: EvidenceCard[]): string {
  if (items.length === 0) return "_无_\n\n";
  return items.map((c, i) => `### ${i + 1}. ${c.title}（${c.type}）\n\n${c.content}\n\n- 能力：${c.abilities.join("、")}\n- 适用：${c.supportedQuestions.join("、")}\n- 风险：${c.usageRisk}\n`).join("\n");
}
...
function mdTrace(items: AgentTraceItem[]): string {
  if (items.length === 0) return "_无_\n\n";
  return items.map((t) => `- ${t.status === "success" ? "✓" : "✗"} ${t.agentName}（${t.durationMs != null ? Math.round(t.durationMs / 1000) + "s" : "?"}）: ${t.summary}`).join("\n") + "\n\n";
}
