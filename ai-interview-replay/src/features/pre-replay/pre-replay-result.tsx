...
export function PreReplayResult({ report, copyText }: PreReplayResultProps) {
  return (
    <div className="space-y-4">
      ...
      <ReportSection title="临场回答诊断" icon="">
        <BulletList items={report.liveAnswerDiagnosis} type="diagnosis" />
      </ReportSection>

      <ReportSection title="冷静回答改进点" icon="">
        <BulletList items={report.calmAnswerImprovements} type="improvement" />
      </ReportSection>

      <ReportSection title="临场损失分析" icon="">
        <BulletList items={report.liveLossAnalysis} type="diagnosis" />
      </ReportSection>

      <ReportSection title="材料证据遗漏" icon="">
        <BulletList items={report.missingEvidence} type="diagnosis" />
      </ReportSection>

      <ReportSection title="导师追问风险" icon="">
      ...
      <ReportSection title="下一次练习建议" icon="">
        <BulletList items={report.nextPracticeAdvice} type="improvement" />
      </ReportSection>
    </div>
  );
}
